...
const ProductDetail = () => {
  ...
  useEffect(() => {
    if (product) {
      let productCopy = Product.slice(); // corrected: slice instead of silce
      productCopy = productCopy.filter(
        (item) => product.category === item.category && item._id !== product._id
      );
      setreleatedProducts(productCopy.slice(0, 5));
    }
  }, [Product, product]);
          ...
          <div className="text-sm w-full md:w-1/2">
            ...
            <div className="flex items-center gap-0.5 mt-1">
              {Array(5)
                ...
                .map((_, i) =>
                  product.rating > i ? (
                    <img key={i} src={assets.sta} alt="Star" />
                  ) : (
                    <svg
                      key={i}
                      width="14"
            ...
            <div className="flex items-center mt-10 gap-4 text-base">
              ...
              <button className="w-full py-3.5 cursor-pointer font-medium bg-primary  text-white hover:bg-indigo-600 transition">
                Buy now
              </button>
            </div>
          </div>
